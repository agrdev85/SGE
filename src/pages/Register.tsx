...
import { Layers, Loader2, ArrowRight, ArrowLeft, Upload, User } from 'lucide-react';
...
const defaultRegistrationFields = [
  ...
  { id: 'scientificLevel', label: 'Nivel Académico', type: 'select', required: false, enabled: true },
...
const countries = ['Cuba', 'México', 'Argentina', 'España', 'Colombia', 'Chile', 'Perú', 'Venezuela', 'Brasil', 'Estados Unidos', 'Ecuador', 'Bolivia', 'Uruguay', 'Paraguay', 'Costa Rica', 'Panamá', 'Guatemala', 'Honduras', 'El Salvador', 'Nicaragua', 'República Dominicana', 'Puerto Rico', 'Canadá', 'Francia', 'Alemania', 'Italia', 'Portugal', 'Reino Unido'];
const affiliations = ['Universidad', 'Centro de Investigación', 'Hospital', 'Empresa Privada', 'Gobierno', 'ONG', 'Institución Educativa', 'Laboratorio', 'Fundación', 'Organismo Internacional', 'Otro'];
const affiliationTypes = ['Entidad Presupuestada', 'Empresa Estatal', 'Empresa Mixta', 'Empresa Privada', 'Cooperativa', 'Organismo Internacional', 'ONG', 'Institución Académica', 'Centro de Investigación', 'Otro'];
const economicSectors = ['Educación', 'Salud', 'Tecnología', 'Agricultura', 'Industria', 'Comercio', 'Turismo', 'Construcción', 'Transporte', 'Telecomunicaciones', 'Energía', 'Finanzas', 'Servicios', 'Otro'];
const participationTypes = ['Ponente', 'Coautor', 'Asistente', 'Organizador', 'Patrocinador', 'Invitado Especial', 'Otro'];
const scientificLevels = ['Estudiante de Pregrado', 'Estudiante de Posgrado', 'Especialista', 'Máster', 'Doctor', 'Investigador Titular', 'Profesor Titular', 'Académico', 'Otro'];
const educationalLevels = ['Técnico Medio', 'Técnico Superior', 'Universitario', 'Posgrado', 'Máster', 'Doctor', 'Postdoctorado'];

const selectOptionsMap: Record<string, string[]> = { country: countries, affiliation: affiliations, affiliationType: affiliationTypes, economicSector: economicSectors, participationType: participationTypes, scientificLevel: scientificLevels, educationalLevel: educationalLevels };

export default function Register() {
  const [searchParams] = useSearchParams();
  const macroEventId = searchParams.get('event');
  const [macroEvent, setMacroEvent] = useState<MacroEvent | null>(null);
  const [formData, setFormData] = useState({ name: '', email: '', password: '', confirmPassword: '', phone: '', idDocument: '', country: '', affiliation: '', affiliationType: '', economicSector: '', participationType: '', scientificLevel: '', educationalLevel: '', gender: 'Masculino', profilePhoto: null as File | null });
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { setFormData(prev => ({ ...prev, profilePhoto: file })); const reader = new FileReader(); reader.onloadend = () => setPhotoPreview(reader.result as string); reader.readAsDataURL(file); } };

  const renderField = (field: any) => {
    const isRequired = field.required;
    if (field.id === 'profilePhoto') return (
      <div key={field.id} className="flex flex-col items-center space-y-3 pb-5 border-b">
        ...
        <div className="relative"><div className="h-24 w-24 rounded-full border-4 border-primary overflow-hidden bg-muted flex items-center justify-center">{photoPreview ? <img src={photoPreview} alt="Preview" className="h-full w-full object-cover" /> : <User className="h-12 w-12 text-muted-foreground" />}</div></div>
        <Label htmlFor="photo" className="cursor-pointer"><div className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-medium flex items-center gap-2 hover:opacity-90 transition-opacity"><Upload className="h-4 w-4" />Seleccionar Archivo</div><Input id="photo" type="file" accept="image/*" onChange={handlePhotoChange} className="hidden" /></Label>
      </div>
    );
    if (field.id === 'gender') return (
      <div key={field.id} className="space-y-3">
        <Label className="text-sm font-medium text-foreground">Género {isRequired && <span className="text-destructive">*</span>}</Label>
        <RadioGroup value={formData.gender} onValueChange={v => updateForm('gender', v)} className="flex gap-6">
          <div className="flex items-center space-x-2"><RadioGroupItem value="Masculino" id="masculino" className="border-primary text-primary" /><Label htmlFor="masculino" className="cursor-pointer font-normal">Masculino</Label></div>
          <div className="flex items-center space-x-2"><RadioGroupItem value="Femenino" id="femenino" className="border-primary text-primary" /><Label htmlFor="femenino" className="cursor-pointer font-normal">Femenino</Label></div>
        </RadioGroup>
      </div>
    );
    if (field.type === 'select' && selectOptionsMap[field.id]) return (
      <div key={field.id} className="space-y-2">
        ...
        <Select value={(formData as any)[field.id] || ''} onValueChange={v => updateForm(field.id, v)} required={isRequired}>
          <SelectTrigger className="border-border focus:border-primary focus:ring-primary"><SelectValue placeholder="Seleccione..." /></SelectTrigger>
          <SelectContent>{selectOptionsMap[field.id].map((opt: string) => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent>
        </Select>
      </div>
    );
    return (
      <div key={field.id} className="space-y-2">
        <Label htmlFor={field.id} className="text-sm font-medium text-foreground">{field.label} {isRequired && <span className="text-destructive">*</span>}</Label>
        <Input id={field.id} type={field.type || 'text'} value={(formData as any)[field.id] || ''} onChange={e => updateForm(field.id, e.target.value)} required={isRequired} className="border-border focus:border-primary focus:ring-primary" />
      </div>
    );
  };
  ...
  let row: any[] = [];
  enabledFields.forEach((f: any) => { if (fullWidthIds.includes(f.id)) { if (row.length > 0) { gridFields.push(row); row = []; } gridFields.push([f]); } else { row.push(f); if (row.length === 2) { gridFields.push(row); row = []; } } });
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                {gridFields.map((row, idx) => {
                  if (row.length === 1 && fullWidthIds.includes(row[0].id)) return renderField(row[0]);
